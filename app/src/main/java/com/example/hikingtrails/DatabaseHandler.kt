...
class DatabaseHandler(context: Context) :
        ...
        fun getTimeMeasurementsForTrailWithTimestamp(trailId: Int): List<Pair<Int, Long>> {
                ...
                val selectQuery = "SELECT time, timestamp FROM $TIME_MEASUREMENT_TABLE_NAME WHERE trail_id = $trailId ORDER BY id"
                val db = this.readableDatabase
                val cursor = db.rawQuery(selectQuery, null)
                if (cursor.moveToFirst()) {
                        do {
                                ...
                                val timestampLong = SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(timestamp)?.time ?: 0
